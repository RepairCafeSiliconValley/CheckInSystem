...
function CheckInForm({ event, onComplete }) {
  ...
  return (
    <div>
      ...
      {error && <div style={{ padding: "8px 12px", background: "#fef3f2", borderRadius: "8px", marginBottom: 14 }}><span style={{ fontFamily: "'Outfit', sans-serif", fontSize: "13px", color: "#b42318" }}>{error}</span></div>}
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f5f6f8", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          <div style={{ display: "inline-block", marginBottom: 24 }}><Logo /></div>
          <Card>
            <div style={{ fontSize: "32px", marginBottom: 12 }}>😕</div>
            <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: "18px", fontWeight: 700, color: "#1d2939", margin: "0 0 8px 0" }}>Invalid Check-In Link</h2>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "14px", color: "#667085", lineHeight: 1.5, margin: 0 }}>This check-in link isn't valid — ask a volunteer for help.</p>
          </Card>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f6f8", fontFamily: "'Outfit', sans-serif" }}>
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "20px 16px 80px" }}>
        {confirmData ? (
          <ConfirmationScreen data={confirmData} onReset={() => setConfirmData(null)} />
        ) : (
          <CheckInForm event={event} onComplete={(data) => setConfirmData(data)} />
        )}
      </div>
