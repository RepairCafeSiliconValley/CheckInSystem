...
export default function CoordinatorQueue({
  ...
  const filtered = visitorGroups.filter((g) => {
    const matchesFilter =
      filter === "all" || g.orders.some((o) => o.status === filter);
    ...
    <div>
        ...
        {filterButtons.map((f) => (
          <button
            ...
            style={{
              ...
              background: filter === f.key ? "#1e3a6e" : "#f0f2f5",
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "32px 16px" }}>
          ...
          >
            {search ? "No results found." : "No visitors in this category."}
          </p>
        </div>
      )}
      ...
      {filtered.map((g) => (
        <Card
          key={g.attendee?.id}
          onClick={() => onSelectVisitor(g.attendee?.id)}
        ...
        >
          ...
          {g.orders.map((o) => (
            ...
            >
              ...
              <Badge
                text={`P${o.priority}`}
                color={o.priority === 1 ? "#1e3a6e" : "#e07850"}
              />
