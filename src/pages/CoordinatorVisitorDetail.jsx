...
import { fetchVisitorDetail, updateAttendee, updateWorkOrder } from "../lib/store";

export default function CoordinatorVisitorDetail({ attendeeId, onBack, onPrint }) {
  ...
  const loadData = async () => {
    try {
      ...
      const edits = {};
      wo.forEach((w) => {
        edits[w.id] = { item_name: w.item_name, category: w.category, description: w.description, fixer_name: w.fixer_name || "" };
      });
      setItemEdits(edits);
    } catch (err) {
  ...
  useEffect(() => { loadData(); }, [attendeeId]);

  if (loading) {
    return <p style={{ fontFamily: "'Outfit', sans-serif", color: "#667085", textAlign: "center", padding: 32 }}>Loading...</p>;
  }

  if (!attendee) {
    return <div><Button variant="ghost" onClick={onBack}>← Back</Button><p>Visitor not found.</p></div>;
  }
  ...
  const saveAll = async () => {
    try {
      await updateAttendee(attendeeId, { name: attName.trim(), email: attEmail.trim() });
  ...
  const editOutcome = async (woId) => {
    await updateWorkOrder(woId, { status: "in-progress", outcome: null });
    await loadData();
  };
        ...
        return (
          <Card key={wo.id} style={{ marginBottom: 16, borderLeft: `4px solid ${wo.priority === 1 ? "#1e3a6e" : "#e07850"}` }}>
            ...
            {(wo.status === "pending" || wo.status === "reviewed") && (
              <>
                <Input label="Item Name" value={e.item_name || ""} onChange={(v) => updateItem(wo.id, "item_name", v)} placeholder="Item name" required />
            ...
            {(wo.status === "in-progress" || wo.status === "completed") && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: "16px", fontWeight: 600, color: "#1d2939", marginBottom: 4 }}>{wo.item_name}</div>
                <Badge text={wo.category} />
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "13px", color: "#667085", margin: "8px 0 0 0", lineHeight: 1.5 }}>{wo.description}</p>
              </div>
            )}
            ...
            {wo.status === "in-progress" && (
              <div style={{ marginTop: 8 }}>
                <Input label="Fixer Name (optional)" value={e.fixer_name || ""} onChange={(v) => updateItem(wo.id, "fixer_name", v)} placeholder="Who worked on this?" />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {OUTCOMES.map((o) => (
                    <button key={o} onClick={() => setOrderOutcome(wo.id, o)}
                      style={{ padding: "10px 12px", borderRadius: "8px", border: "1.5px solid #d0d5dd", background: "#fff", fontFamily: "'Outfit', sans-serif", fontSize: "13px", fontWeight: 500, color: "#475467", cursor: "pointer", transition: "all 0.15s" }}>
            ...
            {wo.status === "completed" && (
              <div style={{ marginTop: 8 }}>
                <div style={{ padding: "10px", background: "#e8f5e9", borderRadius: "8px", textAlign: "center", marginBottom: 8 }}>
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: "14px", fontWeight: 600, color: "#2e7d32" }}>{wo.outcome}</span>
                  {wo.fixer_name && <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: "12px", color: "#667085", marginLeft: 8 }}>by {wo.fixer_name}</span>}
                </div>
                <Button variant="ghost" onClick={() => editOutcome(wo.id)} style={{ fontSize: "13px", padding: "8px 12px" }}>
                  ✏️ Edit Outcome
                </Button>
              </div>
            )}
